# type: ignore
"""
Copied from: https://github.com/RasaHQ/rasa-nlu-examples
"""
    ...
    def _extract_entities(self, message: Message) -> List[Dict[Text, Any]]:
        ...
        for substr, timestamp in hits:
            for match in re.finditer(substr, message.get(TEXT)):
                matches.append(
                    {
                        ENTITY_ATTRIBUTE_TYPE: "datetime",
                        ENTITY_ATTRIBUTE_START: match.start(),
                        ENTITY_ATTRIBUTE_END: match.end(),
                        ENTITY_ATTRIBUTE_VALUE: str(timestamp),
                        "original_date": message.get(TEXT)[match.start() : match.end()],
                        "confidence": 1.0,
